...
def traverse_paths(graph, start_node):
    all_paths = []
    stack = [(start_node, [start_node])]  # Pile pour stocker les chemins partiels

    while stack:
        node, path = stack.pop()
        # Si c'est un nœud terminal, ajouter le chemin complet
...
def algo(json_data):
    g = nx.DiGraph()
      # Pour stocker l'élément précédent
    last_decision_task = []  # Pour stocker la dernière tâche de décision de passerelle
        ...
        if 'Task' in json_data["steps"][i]:
            ...
            if not json_data["steps"][i]['Task']['Gateway Path']:
                ...
                if i == 0:
                    ...
                    g.add_node(current_element)  # Ajouter une arête
                ...
                while last_element_path == None:
                    z+=1
                    if z > 50:
                        print ("None: Infinite Loop avoided")
                        return None
                    for c in range(len(last_decision_task)):

                        all_paths = traverse_paths(g, last_decision_task[0-c])
        ...
        if 'Event' in json_data["steps"][i]:
            ...
            if not json_data["steps"][i]['Event']['Gateway Path']:
                if i == 0:
                    ...
                    g.add_node(current_element)   # Ajouter le noeud si le premier
                    last_element = current_element
                else:
                    g.add_edge(last_element,current_element)
                    last_element = current_element
            else:
               # ce bloc qui va chercher le last element d'un path est utilisé plusieurs fois, surement créer fonction 
                last_element_path= None
                while last_element_path == None:
        ...
        if 'Gateway' in json_data["steps"][i]:
            if i == 0:
                ...
                if json_data["steps"][i]['Gateway']['type'] == "Exclusive Gateways":
                    ...
                    if not json_data["steps"][i]['Gateway']['Previous Gateway Path']:
                    #last_decision_task = str(json_data["steps"][i]['Gateway']['decision task'])
                    
                        # Ajouter les branches pour chaque événement dans la passerelle d'événement
                        ...
                        all_paths = traverse_paths(g, last_decision_task[-1])
                        for path in all_paths:
                            if json_data["steps"][i]['Gateway']['Previous Gateway Path'] in path:
                                last_element = path[-1]
                                break
    ...
    all_pathes = traverse_paths(g, start_node)
    #résultats dans la syntaxe Sketch Miner
    sketches=""
